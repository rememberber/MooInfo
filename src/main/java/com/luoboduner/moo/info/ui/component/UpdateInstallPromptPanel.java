...
import com.luoboduner.moo.info.util.UpdateDownloadManager;
...
/**
 * Bottom-right "update ready" install prompt after a silent download finishes.
 */
@Slf4j
public class UpdateInstallPromptPanel extends JPanel {
    ...
    private void applyLabels() {
        titleLabel.setText("Install update");
        String version = UpdateDownloadManager.getInstance().getVersion();
        if (version != null) {
            versionLabel.setText(version + " is ready");
        }
    ...
    private void applyAccentStyle() {
        ...
        FlatSVGIcon icon = new FlatSVGIcon("icons/down.svg");
    ...
    private void showNotesPopup() {
        ...
        int popupWidth = 360;
        int popupHeight = 300;
        int x = location.x - popupWidth - 12;
        if (x < 16) {
            x = Math.min(screen.width - popupWidth - 16, location.x + actionButton.getWidth() + 12);
        }
    ...
    private void install() {
        ...
        try {
            ...
            log.error("Failed to open update package", e);
            installing = false;
            actionButton.setEnabled(true);
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(this),
                    "Failed to open the installer. You can download it from GitHub Releases.",
                    "Failure",
