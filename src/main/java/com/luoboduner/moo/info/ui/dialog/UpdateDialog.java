...
import com.luoboduner.moo.info.util.DownloadLinkSelector;
...
public class UpdateDialog extends JDialog {
    ...
    public UpdateDialog() {
        ...
        buttonDownloadFromWeb.addActionListener(e -> {
            ...
            try {
                desktop.browse(new URI(UiConsts.GITHUB_RELEASES_URL));
                ...
                () -> {
                    ...
                    if (StringUtils.isEmpty(downloadLinkInfo) || downloadLinkInfo.contains("404: Not Found")) {
                        EdtUtil.run(() -> JOptionPane.showMessageDialog(App.mainFrame,
                                "Failed to fetch download links. Please open the download page.",
                                "Network error",
                                JOptionPane.INFORMATION_MESSAGE));
                        return;
                    }
                    JSONObject parse = JSON.parseObject(downloadLinkInfo);
                    String fileUrl = DownloadLinkSelector.select(parse);
                    if (StringUtils.isBlank(fileUrl)) {
                        EdtUtil.run(() -> JOptionPane.showMessageDialog(App.mainFrame,
                                "No installer is available for this platform. Please open the download page.",
                                "Network error",
                                JOptionPane.INFORMATION_MESSAGE));
                        return;
                    }
