#!/usr/bin/env python3
"""Validate MooInfo Java release metadata and render GitHub Release notes."""
...
DEFAULT_REPO = "rememberber/MooInfo"
...
def java_release_notes(version: str, title: str, log: str) -> str:
    ...
    if not changes:
        raise ValueError(f"MooInfo {version} has no release notes")
    return "\n".join([
        "## English",
        "",
        f"> Product: MooInfo  ",
...
def java_release_info(project_root: Path, tag: str) -> ReleaseInfo:
    ...
    ui_consts = (project_root / "src/main/java/com/luoboduner/moo/info/ui/UiConsts.java").read_text(encoding="utf-8")
    ui_match = re.search(r'APP_VERSION\s*=\s*"([^"]+)"', ui_consts)
    if ui_match is None or ui_match.group(1) != version:
        actual = ui_match.group(1) if ui_match else "missing"
        raise ValueError(f"UiConsts.APP_VERSION must be {version}, got {actual}")

    summary_path = project_root / "src/main/resources/version_summary.json"
    summary = load_json(summary_path)
    if not isinstance(summary, dict) or summary.get("currentVersion") != version:
        raise ValueError(f"version_summary.currentVersion must be {version}")
    version_index = summary.get("versionIndex")
    details = summary.get("versionDetailList")
    if not isinstance(version_index, dict) or version not in version_index:
        raise ValueError(f"version_summary.versionIndex is missing {version}")
    if not isinstance(details, list):
        raise ValueError("version_summary.versionDetailList must be an array")
    detail = next((item for item in details if isinstance(item, dict) and item.get("version") == version), None)
    if detail is None or not isinstance(detail.get("title"), str) or not detail["title"].strip():
        raise ValueError(f"version_summary.versionDetailList is missing title for {version}")
    if not isinstance(detail.get("log"), str) or not detail["log"].strip():
        raise ValueError(f"version_summary.versionDetailList is missing log for {version}")

    title = f"MooInfo {version}"
    notes = java_release_notes(version, detail["title"], detail["log"])
    return ReleaseInfo(version, tag, title, notes, None, bool(parsed.prerelease))
